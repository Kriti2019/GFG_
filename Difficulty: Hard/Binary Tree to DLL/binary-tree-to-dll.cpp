...
class Solution {
  ...
  void convertToDLL(Node* root, Node*& prev, Node*& head) {
        ...
        if (prev == NULL) {
            // This is the leftmost node, hence it becomes the head of the DLL
            head = root;
        } else {
            // Link the previous node (prev) with the current root node
            prev->right = root;
            root->left = prev;
        }

        // Mark the current node as the previous node before moving to the right subtree
        prev = root;
